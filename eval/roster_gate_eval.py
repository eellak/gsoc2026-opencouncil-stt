...
def main(source="proxy"):
    ...
    if source == "real":
        rosters = json.loads((LOOP / "rosters.json").read_text())

        def roster_for(meeting_id, exclude_uid, city_id=None):
            return rosters.get(f"{city_id}/{meeting_id}", [])

        def have_roster(city_id, meeting_id):
            return bool(rosters.get(f"{city_id}/{meeting_id}"))
        sizes = [len(v) for v in rosters.values()]
        print(f"REAL rosters: {len(rosters)} meetings | median size ~"
              f"{sorted(sizes)[len(sizes)//2] if sizes else 0}\n")
    else:
        # leakage-safe proxy: recurring (>=2 utterances) name-like tokens from OTHER utterances
        ...
        def roster_for(meeting_id, exclude_uid, city_id=None, min_uttr=2):
        ...
        print(f"PROXY rosters: {len(meeting_name_uids)} meetings | median size ~"
              f"{sorted(sizes)[len(sizes)//2] if sizes else 0}\n")

    spans_fn = _make_gate("greekbert")
    test = [json.loads(l) for l in (LOOP / "test.jsonl").read_text().splitlines() if l.strip()]
    test = [r for r in test if have_roster(r["city_id"], r["meeting_id"])]
    test_spans = spans_fn([r["input_raw"] for r in test])
    print(f"test rows with a roster: {len(test)}")
    ...
    for gated, slist in (("ungated", [None] * len(test)), ("gated", test_spans)):
        ...
        for r, sp in zip(test, slist):
            ...
            terms = roster_for(r["meeting_id"], r["utterance_id"], r["city_id"])
    ...
    seen, clean = set(), []
    for _, r in ev.iterrows():
        if not have_roster(r["city_id"], r["meeting_id"]):
            continue
        g = str(r["gold_final"]).strip()
        k = (r["city_id"], greek_normalize(g))
        if g and k not in seen:
            seen.add(k)
            clean.append({"city_id": r["city_id"], "meeting_id": r["meeting_id"],
                          "utterance_id": r["utterance_id"], "gold_final": g})
        if len(clean) >= 600:
            break
    ...
    for gated, slist in (("ungated", [None] * len(clean)), ("gated", clean_spans)):
        ...
        for c, sp in zip(clean, slist):
            terms = roster_for(c["meeting_id"], c["utterance_id"], c["city_id"])
...
if __name__ == "__main__":
    ...
    ap.add_argument("--source", choices=["proxy", "real"], default="proxy")
