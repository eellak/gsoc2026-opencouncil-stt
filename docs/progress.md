...
| 3    | Dataset finalisation; publish reproducible dataset on HuggingFace                                             | in progress | `eval/hf_export/` pipeline built + run (spec [hf-dataset-export](specs/hf-dataset-export.md)): **combined sample** 29,893 rows (5,095 review includes + ~4.8k NB2 leftover + ~20k trusted no-edit backbone), ONE frozen speaker-disjoint 80/20 split over the whole sample (val 20.1% of hours), forced-align+VAD boundary pass + backbone align-gate → `data/hf-dataset/public/` (train/val parquet+jsonl, card, split map). Pending: combined boundary pass finishing, human eyeball of overlap/boundary reports, license confirmation, then manual `huggingface-cli upload` |
