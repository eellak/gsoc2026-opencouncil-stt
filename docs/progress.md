...
| 6    | LoRA fine-tuning, hyperparameter sweep (rank, alpha, learning rate)                                           | in progress | turbo sweep ran 6.3/9 configs before the 12h guard stopped it (per-epoch flush kept all 19 rows). Every config beat baseline on BOTH sets — val_corr 24.0→~21, val_reg 23.0→~14 (no regression to manage). LR/rank within eval noise (~654 words); pick `lr1e-4 r32, 2 epochs`. Confirm on large-v3 (sweep was turbo proxy). Sweep + seeds/CIs still pending. **⚠ 2026-07-31: this sweep is void** — it ran with the label-prefix bug, so every config trained on shifted targets. The picks are "selected under the legacy label schema, never revalidated", not confirmed. See [label-prefix bug](reports/2026-07-31-label-prefix-bug.md) |
| 7    | LoRA fine-tuning continued; evaluate on held-out municipalities                                               | done | **Full large-v3 LoRA run finished 2026-07-22** (r32 α64 lr1e-4, 2 epochs, 8h31m/RTX3090, on the ~28.6h combined set). Held-out cities argos+orestiada: **val_corr WER 55.6→37.74** (norm 46.9→29.35, CER 33.0→17.46), **val_reg 24.8→10.46** (norm 15.3→4.87, CER 14.3→3.43) — hard set −32% rel, general speech improved not regressed. Fixed a silent OOM-at-map-79% (cgroup 54GB cap vs in-RAM feature accumulation → stream featcache to disk). Adapter backed up local + published **public** on `opencouncil/whisper-large-v3-el-council-lora`. Report: [reports/2026-07-23-fulltrain-run.md](reports/2026-07-23-fulltrain-run.md). **⚠ 2026-07-25: these deltas are superseded.** A controlled same-stack A/B shows the adapter ties base whisper on general held-out utterances and *regresses* on corrected ones (0.176 vs 0.158) — the gains were baseline-serving artifacts. The run happened, the improvement did not. See [postmortem](handoff/2026-07-25-finetune-eval-postmortem.md). **⚠ 2026-07-31:** this run also carried the [label-prefix bug](reports/2026-07-31-label-prefix-bug.md) — it trained to emit `<\|startoftranscript\|>` with every target shifted one position. Fixed in `00d9235`; the published adapter predates the fix and needs replacing |
| 8    | Target morphological errors and domain terminology                                                            | in progress | Approach changed after the postmortem: try **inference-time contextual biasing** (per-meeting roster as faster-whisper `hotwords`, `eval/controlled_eval/ab_hotwords.py`) before any retrain — it targets names without the onset/spelling regressions the LoRA added |
