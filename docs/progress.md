...
| 3    | Dataset finalisation; publish reproducible dataset on HuggingFace                                             | in progress | `eval/hf_export/` pipeline built + run (spec [hf-dataset-export](specs/hf-dataset-export.md)): live export → filter → seeded speaker split (val 20.2% of hours, leakage guards pass) → forced-align+VAD boundary pass → `data/hf-dataset/public/` (train/val parquet+jsonl, card, split map). Pending: boundary pass finishing, human eyeball of overlap/boundary reports, license confirmation, then manual `huggingface-cli upload` |
