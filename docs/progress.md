...
| 3    | Dataset finalisation; publish reproducible dataset on HuggingFace                                             | in progress | `eval/hf_export/` pipeline built + run (spec [hf-dataset-export](specs/hf-dataset-export.md)): **combined sample** 36,846 rows / 28.6 h after the full-leftover re-split (review includes + NB2 judged-keep leftover + no-edit backbone, all align-gated), ONE frozen speaker-disjoint split (val 21.3% of hours), forced-align+VAD boundary pass → `data/hf-dataset/public/`. **Staged private on HF: `haroldpoi/opencouncil-greek-asr`** (round-trip verified 2026-07-15; publish-gate checks green). Pending: human eyeball of overlap/boundary reports, license confirmation, org write access → org upload per `docs/runbooks/hf-upload.md` |
