...
## Ανοιχτές κατευθύνσεις που ζητήθηκαν στις 2026-08-10

Τέσσερα ερωτήματα του χρήστη, πριν το run των πακέτων. Το ένα εκτελέστηκε· τα τρία
καταγράφονται εδώ ώστε να μη ζουν μόνο σε μια συζήτηση.

- [x] **Πόσα epochs.** Απαντήθηκε με μέτρηση, όχι με γνώμη: η συνταγή έκανε **ακριβώς 2.00
      epochs** (7.242 βήματα × 8 κλιπ ÷ 28.967 κλιπ) πάνω σε 22.47 ώρες, και **το 89-91%
      κάθε βήματος ήταν padding**, γιατί ένα κλιπ 2.79 δευτ. κάθεται μέσα σε θυρίδα 30
      δευτ. Πληρώναμε 483 ώρες υπολογισμού για να δείξουμε 45 ώρες ομιλίας. Τα πακέτα το
      ρίχνουν στο 13%, και τα βήματα πάγωσαν στα **2.600 ≈ 5.4 epochs**.
      Δες [runbook](runbooks/2026-08-10-unattended-packing-run.md).

- [ ] **BERTScore δίπλα στο WER.** Λογικό ως συμπληρωματικό, με ελληνικό BERT
      (`nlpaueb/bert-base-greek-uncased-v1`). **Η επιφύλαξη πρέπει να ειπωθεί πριν μπει:**
      δεν λύνει το πρόβλημα της αναφοράς. Αν η αναφορά χάνει 17.5% των λέξεων, το
      BERTScore θα ανταμείβει τη συμφωνία με λάθος κείμενο, απλώς πιο επιεικώς. Χρήσιμο
      για «είπε το ίδιο με άλλες λέξεις», **άχρηστο για παραλείψεις**, που είναι το
      εύρημα που γέννησε όλη αυτή τη δουλειά.

- [ ] **Περισσότερα ελληνικά δεδομένα, π.χ. Βουλή.** Η καλύτερη από τις τέσσερις ιδέες,
      γιατί χτυπάει τον πραγματικό περιορισμό. Τρεις έλεγχοι πριν επενδυθεί χρόνος:
      **άδεια χρήσης**· αν τα πρακτικά είναι **ευθυγραμμισμένα με τον ήχο** ή μόνο κείμενο·
      και **πόσο επιμελημένα** είναι. Το τρίτο είναι το επικίνδυνο: τα πρακτικά της Βουλής
      παραδοσιακά «καθαρίζονται», που είναι ακριβώς η παθολογία του δικού μας transcript.
      Καθαρός ήχος με μαγειρεμένο κείμενο διδάσκει παράλειψη.

- [ ] **Βιβλιογραφία και βέλτιστες πρακτικές.** Αγγίχτηκε αποσπασματικά (Timmel et al. 2024,
      ivrit.ai 2025) μέσα από την προδήλωση του σχήματος παραθύρου, και ήδη μια φορά
      **διαβάστηκε πιο δυνατά απ' ό,τι λέει**. Χρειάζεται συστηματικό πέρασμα, όχι
      ευκαιριακές αναφορές που στηρίζουν ό,τι θέλουμε να τρέξουμε.
