...
1. **Test fusion against the live pipeline.** Three ASR accounts per meeting may cost too much, so the real question is whether two systems are enough. I have now run the free screen: all 18 configurations, every threshold fitted leave-one-city-out, headline from out-of-fold predictions only. Two systems do win on WER. The best pair is our adapter with Soniox at 0.12238 against 0.13858 for the better of the two alone, and no two-system cell reaches the three-system 0.10046. But every cell that wins on WER fails the insertion guard I froze before running it, by trading deletions for insertions, so on the preregistered rule none of them ships. The mechanism makes that worse rather than better: the winning arm earns its gain by discarding 4,579 tokens Soniox emitted, and a separate experiment measured 40.8% of Soniox insertions as real speech the published reference omits. Against a human who listened, this arm may be worse than what it beats on paper.
...
3. **Find out why whole passages disappear.** 36.3% of what v2 deletes vanishes in runs of five or more consecutive words, against 18.6% for Scribe. I did not find a cause, but I did change the question. The property is inherited from whisper rather than made by our training: base `whisper-large-v3` sits at 32.0% with none of our data, the v1 adapter at 42.3%, and v2 at 36.3%, so fine-tuning moved the volume of deletions and left their shape alone. The unit that disappears is a whole published utterance or speaker turn of about two seconds, not a random span: a quarter of the runs align exactly to utterance boundaries against a 2.5% baseline. And 19.1% of those tokens are spans no other system wrote either, inside human-edited reference text, which is a property of the reference rather than of any model. The one measurement I would buy next is a single word-timestamped re-decode, to see whether the decoder's own segment boundaries fall on the edges of the runs.
